...
class Hopkins_Hub_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		foreach($hub_articles['articles'] as $hub_article) { ?>
				<article class="row" aria-label="<?php echo $hub_article['headline']; ?>">
						<a href="<?php echo $hub_article['url']; ?>">
							<img src="<?php echo $hub_article['_embedded']['image_thumbnail'][0]['sizes'][$image_size]; ?>" alt="<?php echo $hub_article['headline']; ?>" />
...
function wp_widget_custom_rss_output( $rss, $args = array() ) {
	...
	foreach ( $rss->get_items(0, $items) as $item ) {
		...
		if ( $link == '' ) {
			echo "<article><h5>{$date}</h5><p><strong>$title</strong><br>{$summary}{$author}</p></article>";
		} else {
			echo "<article><h5>{$date}</h5><p><strong><a class='rsswidget' href='$link' title='$desc'>$title</a></strong><br>{$summary}{$author}</p></article>";
		}
	}
	$rss->__destruct();
	unset($rss);
